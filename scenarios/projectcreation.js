...
export function configureProjectAvailability(clientToken, projectId) {
  ...
  const payload = {
    ...
    // NOTE: The fields below are now managed by Super Admin at the
    // organisation level and must NOT be sent from this script.
    // Sending them causes a 400/crash because the backend rejects
    // client-admin-scoped requests that include super-admin-only fields.
...
export function importCandidatesCsv(clientToken, projectId) {
  ...
  let queued = 0;
  let totalRows = 0;
  const importBody = parseResponseBody(res);
  if (importBody) {
    const data = importBody.data || importBody;
